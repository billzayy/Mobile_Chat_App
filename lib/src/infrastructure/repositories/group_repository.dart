...
class GroupRepository implements GroupService {
  ...
  @override
  Future<ApiResponse<List<GroupModel>>> getAllGroup(int userID) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ApiResponse<List<GroupModel>>.error(
  ...
  @override
  Future<ApiResponse<String>> updateFroup(Map<String, dynamic> param) async {
    try {
      final response = await apiClient.dio.put('/group/update', data: param);
      if (response.statusCode == 200) {
        final data = response.data['data'];
  ...
  @override
  Future<ApiResponse<GroupModel>> getGroup(int idGroup) async {
    try {
      final response = await apiClient.dio.get(
        '/group/get-specific-group?idGroup=$idGroup',
      );
      if (response.statusCode == 200) {
        final data = response.data['data'];
        final group = GroupModel.fromJson(data);
